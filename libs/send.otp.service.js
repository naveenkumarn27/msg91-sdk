...
class SendOtpService {
    ...
    static generateOTP(aLength) {
        ...
        if (aLength < 4) {
            console.log("OTP length must be greater than 4 to avoid security breaches")
        }

        let lowDigits = Math.pow(10, aLength - 1)
        let highDigits = 9 * Math.pow(10, aLength - 1)
    ...
    constructSendOTPRequest(aSendOtpService, aOptions) {
        ...
        if (aOptions.extra_param && aOptions.extra_param.length > 0) {
            sendOTPath += "&email=" + JSON.stringify(aOptions.extra_param)
        }
        ...
        if (aSendOtpService.otpTemplateId && aSendOtpService.otpTemplateId.length > 0) {
            sendOTPath += "&template_id=" + aSendOtpService.otpTemplateId
        } else {
            sendOTPath += "&message=" + aSendOtpService.messageTemplate.replace("{{otp}}", otp)
        }
